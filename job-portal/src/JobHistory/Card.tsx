...
const Card = (props: any) => {
    return (
        <Link to="/jobs" className="bg-mine-shaft-900 p-4 w-full sm:w-64 md:w-72 flex flex-col gap-3 rounded-xl hover:shadow-[0_0_5px_1px_yellow]">
            <div className="flex justify-between ">
                <div className="flex gap-2 items-center">
                    <div className="p-2 bg-mine-shaft-800 rounded-md">
                        <img className="h-7 " src={`/JobPortalResources/Icons/${props.company}.png`} alt="" />
                    </div>
                ...
                {!props.saved ? <IconBookmark className="text-mine-shaft-300 cursor-pointer" stroke={1.5} /> :
                    <IconBookmarkFilled stroke={1.5} className="text-bright-sun-400 cursor-pointer" />}
            </div>
...

            {
                props.interviewing && <div className="flex gap-1  text-sm items-center">
                    <IconCalendarMonth className="text-bright-sun-400 w-5 h-5" stroke={1.5} /> Sun, 30 August &bull; <span className="text-mine-shaft-400">10:00</span>
                </div>
            }
        </Link>
    )
